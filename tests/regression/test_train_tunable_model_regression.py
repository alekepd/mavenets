...
EXPECTED_FILE_ANNEALING = FIXTURES_DIR / "train_tunable_model_annealing_expected.npz"
...
def _get_model_state_dict_as_array(
    model: LinearTuner[Tensor],
) -> npt.NDArray[np.float64]:
    """Extract model state dict as a flat numpy array for comparison.
    ...
    Returns:
    -------
    Flattened numpy array of all model parameters.

    """
    state_dict = model.state_dict()
    all_params = []
    for key in sorted(state_dict.keys()):
        all_params.append(state_dict[key].cpu().numpy().flatten())
    return np.concatenate(all_params)


def _run_deterministic_training_with_annealing() -> (
    Tuple[
        LinearTuner[Tensor],
        TensorDataset,
        TensorDataset,
    ...
    """Run training with loss param annealing and report_datasets.

    This tests additional behavior:
    - Loss parameter annealing from 1.0 to 0.0
    - report_datasets functionality
    - Model weights after training

    Returns:
    -------
    Tuple of (model, train_dataset, valid_dataset, report_dataset,
              best_epoch, best_val, loss_table).

    """
    # Set all seeds for reproducibility
    torch.manual_seed(RANDOM_SEED + 5000)
    ...
    # Additional report dataset with different seed
    report_dataset = _create_deterministic_dataset(
        n_samples=17,  # Different size, also incomplete batch (17 % 7 = 3)
        n_features=N_FEATURES,
        n_heads=N_HEADS,
        seed_offset=700,
    )
    ...
    # Create model
    torch.manual_seed(RANDOM_SEED + 6000)
    base_model = MLP(
        in_size=N_FEATURES,
        out_size=1,
        hidden_sizes=[],
    ...
    model = LinearTuner(base_model, n_heads=N_HEADS)

    # Create optimizer with fixed seed
    torch.manual_seed(RANDOM_SEED + 7000)
    optimizer = torch.optim.SGD(model.parameters(), lr=LEARNING_RATE)

    # Train with loss param annealing and report_datasets
    best_epoch, best_val, table = train_tunable_model(
        model=model,
        optimizer=optimizer,
        device="cpu",
        ...
        train_dataset=train_dataset,
        valid_dataset=valid_dataset,
        report_datasets={"test_report": report_dataset},
        train_loss_function=mixed_MSE,
        report_loss_function=mixed_MSE,
        train_batch_size=TRAIN_BATCH_SIZE,
        reporting_batch_size=REPORTING_BATCH_SIZE,
        ...
        end_loss_param=0.0,  # Annealing from 1.0 to 0.0
        loss_param_ramp_size=15,  # Ramp over 15 epochs
        grad_clip=1e3,
        progress_bar=False,
        patience=100,
        graph=False,
...
@pytest.mark.regression
class TestTrainTunableModelAnnealingRegression:
    """Regression tests for train_tunable_model with loss param annealing.

    These tests cover additional behavior not tested in the base regression tests:
    - Loss parameter annealing schedule
    - report_datasets functionality
    - Model weights after training
    """
    ...
    def test_predictions_match_expected(self) -> None:
        """Test that trained model predictions match stored expected values."""
        if not EXPECTED_FILE_ANNEALING.exists():
            pytest.skip(
                f"Expected file {EXPECTED_FILE_ANNEALING} not found. "
                "Run with REGENERATE_BASELINE=1 to generate."
            )
        ...
        (
            ...
            _best_val,
            _loss_table,
        ) = _run_deterministic_training_with_annealing()

        # Load expected values
        expected = np.load(EXPECTED_FILE_ANNEALING)

        # Get predictions
        train_preds = _get_model_predictions(model, train_ds)
        valid_preds = _get_model_predictions(model, valid_ds)
        report_preds = _get_model_predictions(model, report_ds)
        ...
        np.testing.assert_allclose(
            report_preds,
            expected["report_predictions"],
            rtol=1e-5,
            atol=1e-6,
            err_msg="Report dataset predictions do not match expected values",
        )

    def test_model_weights_match_expected(self) -> None:
        """Test that model weights after training match expected values."""
        if not EXPECTED_FILE_ANNEALING.exists():
            pytest.skip(
                f"Expected file {EXPECTED_FILE_ANNEALING} not found. "
                "Run with REGENERATE_BASELINE=1 to generate."
        ...
        # Get model weights
        model_weights = _get_model_state_dict_as_array(model)

        # Compare weights
        np.testing.assert_allclose(
            model_weights,
            expected["model_weights"],
            rtol=1e-5,
            atol=1e-6,
            err_msg="Model weights do not match expected values",
        )

    def test_loss_table_includes_report_dataset(self) -> None:
        """Test that loss table includes report_datasets column."""
        if not EXPECTED_FILE_ANNEALING.exists():
            pytest.skip(
                f"Expected file {EXPECTED_FILE_ANNEALING} not found. "
                "Run with REGENERATE_BASELINE=1 to generate."
        ...
        # Compare full loss table (includes report_datasets column)
        np.testing.assert_allclose(
            loss_table,
            expected["loss_table"],
            rtol=1e-5,
    ...
    def test_loss_param_annealing_recorded(self) -> None:
        """Test that loss_param values show annealing in loss table."""
        if not EXPECTED_FILE_ANNEALING.exists():
            pytest.skip(
                f"Expected file {EXPECTED_FILE_ANNEALING} not found. "
                "Run with REGENERATE_BASELINE=1 to generate."
        ...
        # loss_param is column index 0 in the table
        loss_params = loss_table[:, 0]
        expected_loss_params = expected["loss_table"][:, 0]
        ...
        np.testing.assert_allclose(
            ...
            rtol=1e-5,
            atol=1e-6,
            err_msg="Loss param annealing values do not match expected",
        )

        # Also verify annealing happened (first > last)
        assert loss_params[0] > loss_params[-1], (
            f"Loss param should decrease: {loss_params[0]} -> {loss_params[-1]}"
        )

    def test_generate_baseline(self) -> None:
        """Generate baseline expected outputs for annealing test.
        ...
        # Save expected values
        np.savez(
            EXPECTED_FILE_ANNEALING,
            train_predictions=train_preds,
            valid_predictions=valid_preds,
            report_predictions=report_preds,
        ...
        # Verify the file was created
        assert EXPECTED_FILE_ANNEALING.exists(), (
            f"Failed to create {EXPECTED_FILE_ANNEALING}"
        )

        # Print summary for user verification
        print(f"\nBaseline saved to {EXPECTED_FILE_ANNEALING}")  # noqa: T201
        print(f"  best_epoch: {best_epoch}")  # noqa: T201
        print(f"  best_val: {best_val:.6f}")  # noqa: T201
        print(f"  train_predictions shape: {train_preds.shape}")  # noqa: T201
        print(f"  valid_predictions shape: {valid_preds.shape}")  # noqa: T201
        print(f"  report_predictions shape: {report_preds.shape}")  # noqa: T201
        print(f"  model_weights shape: {model_weights.shape}")  # noqa: T201
        print(f"  loss_table shape: {loss_table.shape}")  # noqa: T201
