...
This model uses per-residue PCA of pretrained t5 embeddings as featurization.
...
PCA_COMPONENTS: Final = 5
# T5 adds an EOS token, so seq_len_out = 201 + 1 = 202
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    for layer_sel, wdecay, lr, dropout in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_dropout{}_base_nulltuner_pcadim{}_t5residuepca.csv".format(
