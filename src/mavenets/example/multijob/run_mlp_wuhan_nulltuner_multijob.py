...
The model is trained on the 'wuhan' dataset only.
...
def test_mlp(
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['wuhan'], val_specs=['wuhan'], device='cuda', feat_type="onehot")
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    for layer_sel,wdecay,lr in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_wuhan_nulltuner.csv".format(repr(layer_sel), wdecay, lr)
