...
The model is trained on the 'base_mutsplit_1234' dataset only; this corresponds to the
1- through 4-mutations (inclusive) of the base dataset.
...
def test_mlp(
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['base_mutsplit_1234'], val_specs=['base_mutsplit_1234'], device='cuda', feat_type="onehot")
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    for layer_sel,wdecay,lr in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_base1234mut_nulltuner.csv".format(repr(layer_sel), wdecay, lr)
