...
The model is trained on the 'small_base' dataset only; this corresponds to the
a subset of the base dataset (with fewer examples)
...
def test_mlp(
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['small_base'], val_specs=['small_base'], device='cuda', feat_type="onehot")
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    for layer_sel,wdecay,lr in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_smallbase_nulltuner.csv".format(repr(layer_sel), wdecay, lr)
