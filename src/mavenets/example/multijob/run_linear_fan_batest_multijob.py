"""Train fan-linear using prediction accuracy using multiple GPUs.

The model is first trained on datasets. Then, the parameters fo the fans are frozen
the remaining parameters are reinitialized, and the model is trained on all but the BA datasets.
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    options = list(product(weight_decay_ops, fan_size_ops, learning_rate_ops))
    tasks = get_tasks(data=options,
                      replica=replica,
                      total_n_replicas=total_n_replicas,
                      shuffle=True)

    for wdecay,fsize,lr in tasks:
        name = "linear_wdecay{}_fansize{}_lr{}_batest.csv".format(wdecay,
                                                                  fsize,
                                                                  lr)
        if Path(name).is_file():
            continue
        epoch, val, table = test_linear(
            weight_decay=wdecay,
            fan_size=fsize,
            learning_rate=lr,
        )
