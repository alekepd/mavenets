...
The model is trained on the 'small_base' dataset only.
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    train_dataset, valid_dataset = get_datasets(device=DEVICE, train_specs=['small_base'], val_specs=['small_base'], feat_type="t5")
    ...
    for layer_sel,wdecay,lr,dropout in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_dropout{}_smallbase_nulltuner.csv".format(repr(layer_sel), wdecay, lr, dropout)
