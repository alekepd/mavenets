...
The model is trained on the 'e484k' dataset only.
...
def test_mlp(
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['E484K'], val_specs=['E484K'], device='cuda', feat_type="onehot")
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    for layer_sel,wdecay,lr in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_E484K_nulltuner.csv".format(repr(layer_sel), wdecay, lr)
