...
The model is trained on the 'small_base_mutsplit_12' dataset only; this corresponds to the
...
def test_mlp(
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['small_base_mutsplit_12'], val_specs=['small_base_mutsplit_12'], device='cuda', feat_type="onehot")
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    for layer_sel,wdecay,lr in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_smallbase12mut_nulltuner.csv".format(repr(layer_sel), wdecay, lr)
