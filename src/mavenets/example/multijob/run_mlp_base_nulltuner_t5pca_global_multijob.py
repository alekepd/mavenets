...
This model uses global PCA of pretrained t5 embeddings as featurization.
...
PCA_COMPONENTS: Final = 50*200
# Global PCA flattens (n, seq_len, 1024) -> (n, seq_len*1024) and reduces to
# (n, PCA_COMPONENTS), so in_size is just the number of components.
IN_SIZE: Final = PCA_COMPONENTS
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    options = list(
        product(layer_ops, weight_decay_ops, learning_rate_ops, dropout_opts)
    )
    tasks = get_tasks(
        data=options,
        replica=replica,
    ...
    for layer_sel, wdecay, lr, dropout in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_dropout{}_base_nulltuner_pcadim{}_t5globalpca.csv".format(
            repr(layer_sel), wdecay, lr, dropout, PCA_COMPONENTS
        )
