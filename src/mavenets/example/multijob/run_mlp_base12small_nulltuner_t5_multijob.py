...
The model is trained on the 'small_base_mutsplit_12' dataset only.
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    train_dataset, valid_dataset = get_datasets(device=DEVICE, train_specs=['small_base_mutsplit_12'], val_specs=['small_base_mutsplit_12'], feat_type="t5")
    ...
    for layer_sel,wdecay,lr,dropout in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_dropout{}_basemut12small_nulltuner.csv".format(repr(layer_sel), wdecay, lr, dropout)
