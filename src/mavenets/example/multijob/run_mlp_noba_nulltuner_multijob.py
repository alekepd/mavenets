...
The model is trained on the all data except that from the BA1 and BA2 datasets.
...
def scan(replica: int, total_n_replicas: int) -> None:
    ...
    for layer_sel,wdecay,lr in tasks:
        name = "mlp_l{}_wdecay{}_lr{}_noba_nulltuner.csv".format(repr(layer_sel), wdecay, lr)
