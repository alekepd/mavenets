"""Evaluate a fixed MLP architecture on test sets while only training on B1351 data.
...
def test_mlp(
    ...
    A MLP is trained on data from the B1351 experiment. No experimental head is used.
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['B1351'], val_specs=['B1351'], device=DEVICE, feat_type="onehot")
...
def run(attempt_seeds: Tuple= (1234231, 54636, 2931243)) -> None:
    ...
    core_name = "TESTEVAL_mlp_l{}_wdecay{}_learningrate{}_B1351only_nulltuner_multiattempt".format(repr(layer_sel), wdecay, lr, n_epochs)
