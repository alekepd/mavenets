"""Evaluate a fixed MLP architecture on test sets while only training on single mutation base data.
...
def test_mlp(
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['base_mutsplit_1'], val_specs=['base_mutsplit_1'], device=DEVICE, feat_type="onehot")
...
def run(attempt_seeds: Tuple= (1234231, 54636, 2931243)) -> None:
    ...
    core_name = "TESTEVAL_mlp_l{}_wdecay{}_learningrate{}_nepochs{}_basemut1only_nulltuner_multiattempt".format(repr(layer_sel), wdecay, lr, n_epochs)
