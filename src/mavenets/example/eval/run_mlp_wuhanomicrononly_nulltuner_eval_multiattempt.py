"""Evaluate a fixed MLP architecture on test sets while only training on wuhan omicron data.
...
def test_mlp(
    ...
    A MLP is trained on data from the wuhan omicron experiment. No experimental head is used.
    ...
    # collate wuhan_omicron dataset
    train_dataset, valid_dataset = get_datasets(train_specs=['wuhan_omicron'], val_specs=['wuhan_omicron'], device=DEVICE, feat_type="onehot")
...
def run(attempt_seeds: Tuple= (1234231, 54636, 2931243)) -> None:
    ...
    core_name = "TESTEVAL_mlp_l{}_wdecay{}_learningrate{}_wuhanomicrononly_nulltuner_multiattempt".format(repr(layer_sel), wdecay, lr, n_epochs)
