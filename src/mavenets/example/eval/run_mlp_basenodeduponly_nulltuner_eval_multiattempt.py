"""Evaluate a fixed MLP architecture on test sets while only training on non-deduplicated base data.

This experiment runs on the base dataset before any deduplication processing is run.
...
def test_mlp(
    ...
    n_epochs: int = 1000,
    grad_clip: float = 300,
) -> Tuple[int, float, torch.nn.Module, pd.DataFrame, pd.DataFrame]:
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['base_dedup'], val_specs=['base_dedup'], device=DEVICE, feat_type="onehot")
...
def run(attempt_seeds: Tuple[int, ...]= (1234231, 54636, 2931243)) -> None:
    ...
    core_name = "TESTEVAL_mlp_l{}_wdecay{}_learningrate{}_nepochs{}_basenodeduponly_nulltuner_multiattempt".format(repr(layer_sel), wdecay, lr, n_epochs)
