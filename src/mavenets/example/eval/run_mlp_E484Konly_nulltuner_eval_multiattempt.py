"""Evaluate a fixed MLP architecture on test sets while only training on E484K data.
...
def test_mlp(
    ...
    A MLP is trained on data from the E484K experiment. No experimental head is used.
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['E484K'], val_specs=['E484K'], device=DEVICE, feat_type="onehot")
...
def run(attempt_seeds: Tuple= (1234231, 54636, 2931243)) -> None:
    ...
    core_name = "TESTEVAL_mlp_l{}_wdecay{}_learningrate{}_E484Konly_nulltuner_multiattempt".format(repr(layer_sel), wdecay, lr, n_epochs)
