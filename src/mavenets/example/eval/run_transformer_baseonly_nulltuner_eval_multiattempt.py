...
def helper(seed: int, **kwargs):
    ...
    best_epoch, _, _, _, _ = test_transformer(
            **kwargs
    )
    ...
    torch.manual_seed(seed)
    _, _, model, training_report, test_pred = test_transformer(
            **new_kwargs
    )
...
def run(attempt_seeds: Tuple= (1234231, 54636, 2931243)) -> None:
    ...
    core_name = "TESTEVAL_b{}_h{}_e{}_wdecay{}_mlpdrop{}_mhadrop{}_flayers{}_fdrop{}_baseonly_multiattempt".format(
        n_blocks,
        n_heads,
        emb_size,
        weight_decay,
