...
Uses per-residue PCA of pretrained T5 embeddings as featurization.
...
PCA_COMPONENTS: Final = 50
T5_SEQ_LEN: Final = 202
IN_SIZE: Final = T5_SEQ_LEN * PCA_COMPONENTS
...
def run(attempt_seeds: Tuple = (1234231, 54636, 2931243)) -> None:
    ...
    train_dataset, valid_dataset, test_data = get_datasets(
        device=DEVICE,
        train_specs=["base"],
        val_specs=["base"],
        feat_type="t5_pca",
    ...
    core_name = "TESTEVAL_mlp_t5residuepca_l{}_wdecay{}_learningrate{}_nepochs{}_dropout{}_batchsize{}_pcadim{}_baseonly_nulltuner_multiattempt".format(
        repr(layer_sel), wdecay, lr, n_epochs, dropout, batch_size, PCA_COMPONENTS
    )
