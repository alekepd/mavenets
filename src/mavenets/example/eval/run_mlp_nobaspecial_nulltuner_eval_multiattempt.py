...
def test_mlp(
    ...
    """
    # train is all but BA1 and BA2
    non_ba_specs = [x for x in CORE_DATA_SPECS if x.name not in ["BA1","BA2"]]

    # val is ba1
    train_dataset, valid_dataset = get_datasets(train_specs=non_ba_specs,
                                                val_specs=["BA1"],
                                                device=DEVICE,
                                                feat_type="onehot")
...
def run(attempt_seeds: Tuple= (1234231, 54636, 2931243)) -> None:
    ...
    layer_sel =  [256, 8]
    wdecay = 0.001
    n_epochs = 200 # this the max number of epochs considered; early stopping is used.
    lr = 1e-4
    record = {}
    ...
    core_name = "TESTEVAL_mlp_l{}_wdecay{}_learningrate{}_nobaspecial_nulltuner_multiattempt".format(repr(layer_sel), wdecay, lr)
