"""Evaluate a fixed MLP architecture on test sets while only training on alternatively-split base data.

Experiments are run on the base_trainvaltest-rng789 split. Note that this split also redefines test data,
so the evaluation is performed on every single test set to make things comparable.
...
def test_mlp(
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['base_trainvaltest-rng789'], val_specs=['base_trainvaltest-rng789'], device=DEVICE, feat_type="onehot")
    ...
    # get test data
    _, _, eval_data = get_datasets(device=DEVICE, feat_type="onehot", include_test=True, test_specs=DATA_SPECS)
...
def run(attempt_seeds: Tuple= (1234231, 54636, 2931243)) -> None:
    ...
    core_name = "TESTEVAL_mlp_l{}_wdecay{}_learningrate{}_nepochs{}_baserng789only_nulltuner_multiattempt".format(repr(layer_sel), wdecay, lr, n_epochs)
