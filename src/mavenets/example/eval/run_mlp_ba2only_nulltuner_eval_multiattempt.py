"""Evaluate a fixed MLP architecture on test sets while only training on ba2 data.
...
def test_mlp(
    ...
    A MLP is trained on data from the ba2 experiment. No experimental head is used.
    ...
    train_dataset, valid_dataset = get_datasets(train_specs=['BA2'], val_specs=['BA2'], device=DEVICE, feat_type="onehot")
...
def run(attempt_seeds: Tuple= (1234231, 54636, 2931243)) -> None:
    ...
    core_name = "TESTEVAL_mlp_l{}_wdecay{}_learningrate{}_ba2only_nulltuner_multiattempt".format(repr(layer_sel), wdecay, lr, n_epochs)
