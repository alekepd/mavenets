"""Timing comparison of one-hot vs T5 featurization.
...
def run_benchmark(
    ...
    for batch_size in batch_sizes:
        ...
        # many iterations inside a single timing call and report per-call time.
        onehot_inner_repeats = 1000
        onehot_mean = time_fn(
            f"One-hot encoding (x{onehot_inner_repeats} inner iters, on {device})",
            lambda: [featurize_onehot(int_encoded_dev) for _ in range(onehot_inner_repeats)],
            n_repeats,
            device,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Time one-hot vs T5 protein sequence featurization."
    )
    parser.add_argument(
        "--device",
        type=str,
